...
MitsubaShape<Float, Spectrum>::MitsubaShape(mitsuba::ref<Mesh> mesh)
    : size_(convert<Float, Spectrum, inet::Coord>(mesh->bbox()))
    , mesh_(mesh) {
}
...
bool MitsubaShape<Float, Spectrum>::computeIntersection(
    ...
    ray.d /= len;
    ray.maxt = fromScalar<Float>(lenScalar);
    ...
    intersection1 = convert<Float, Spectrum, inet::Coord>(si.p);
    normal1 = convert<Float, Spectrum, inet::Coord>(si.n);

    Ray3f ray2 = ray;
    ray2.o = si.p + ray.d * advancementEpsilon;

    ray2.maxt = static_cast<Float>(lenScalar - toScalar(si.t) - advancementEpsilon);
    ...
    if (drjit::all_nested(pi2.is_valid())) {
        ...
        intersection2 = convert<Float, Spectrum, inet::Coord>(si2.p);
        normal2 = convert<Float, Spectrum, inet::Coord>(si2.n);
    } else {
