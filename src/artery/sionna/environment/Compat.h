#pragma once

#include <artery/sionna/bridge/SionnaBridge.h>

#include <mitsuba/core/fwd.h>
#include <inet/common/geometry/common/Coord.h>

namespace artery::sionna {

    template <typename Float, typename Spectrum, typename Value>
    struct impl<Float, Spectrum, inet::Coord, mitsuba::Point<Value, 3>, void> {
        static inet::Coord convert(const mitsuba::Point<Value, 3>& value) {
            return inet::Coord(
                toScalar(value[0]),
                toScalar(value[1]),
                toScalar(value[2]));
        }
    };

    template <typename Float, typename Spectrum, typename Value>
    struct impl<Float, Spectrum, inet::Coord, mitsuba::Vector<Value, 3>, void> {
        static inet::Coord convert(const mitsuba::Vector<Value, 3>& value) {
            return inet::Coord(
                toScalar(value[0]),
                toScalar(value[1]),
                toScalar(value[2]));
        }
    };

    template <typename Float, typename Spectrum, typename Value>
    struct impl<Float, Spectrum, inet::Coord, mitsuba::Normal<Value, 3>, void> {
        static inet::Coord convert(const mitsuba::Normal<Value, 3>& value) {
            return inet::Coord(
                toScalar(value[0]),
                toScalar(value[1]),
                toScalar(value[2]));
        }
    };

    template <typename Float, typename Spectrum, typename Value>
    struct impl<Float, Spectrum, inet::Coord, mitsuba::BoundingBox<mitsuba::Point<Value, 3>>, void> {
        static inet::Coord convert(const mitsuba::BoundingBox<mitsuba::Point<Value, 3>>& value) {
            return artery::sionna::convert<Float, Spectrum, inet::Coord>(value.max - value.min);
        }
    };

} // namespace artery::sionna
