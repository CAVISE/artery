...
class CommunicationManager : public std::enable_shared_from_this<CommunicationManager<ArteryMessageType, OpencdaMessageType>>
{
private:
    struct Private {
    };
...
public:
     ...
     */
    static std::shared_ptr<CommunicationManager> create(const std::string& address, std::size_t queueMaxSize);

    /**
     * @brief This constructor is not part of public API.
     */
    CommunicationManager(const std::string& address, std::size_t queueMaxSize, Private access);
...
template <typename ResultType>
template <typename... Args>
cavise::Result<ResultType>::Result(Args&&... args) : result_(std::forward<Args>(args)...), error_("")
{
}

template <typename ResultType>
cavise::Result<ResultType>::Result(const std::string& message) : error_(message), result_(std::nullopt)
{
}

template <typename ArteryMessageType, typename OpencdaMessageType>
std::shared_ptr<cavise::CommunicationManager<ArteryMessageType, OpencdaMessageType>>
cavise::CommunicationManager<ArteryMessageType, OpencdaMessageType>::create(const std::string& address, std::size_t queueMaxSize)
{
...
cavise::CommunicationManager<ArteryMessageType, OpencdaMessageType>::CommunicationManager(
    const std::string& address, std::size_t queueMaxSize, Private /* access */
    ) :
    qMaxSize_(queueMaxSize), address_(address), socket_(context_, zmq::socket_type::pair)
{
}
...
template <typename ArteryMessageType, typename OpencdaMessageType>
cavise::Result<typename cavise::CommunicationManager<ArteryMessageType, OpencdaMessageType>::opencda_message_t>
cavise::CommunicationManager<ArteryMessageType, OpencdaMessageType>::collect()
{
    ...
    if (!opencdaMessage_.get()) {
        ...
        worker_->cv.wait(unique_lock, [this]() { return opencdaMessage_.get(); });
    }
...
template <typename ArteryMessageType, typename OpencdaMessageType>
typename cavise::CommunicationManager<ArteryMessageType, OpencdaMessageType>::artery_message_t
cavise::CommunicationManager<ArteryMessageType, OpencdaMessageType>::joinCav()
{
    std::unique_lock<std::mutex> unique_lock(worker_->mutex);
    ...
    for (auto& pair : arteryCavMapper_) {
        if (!pair.second.size()) {
            worker_->cv.wait(unique_lock, [&pair]() { return pair.second.size(); });
        }
...
template <typename ArteryMessageType, typename OpencdaMessageType>
cavise::CommunicationManager<ArteryMessageType, OpencdaMessageType>::Worker::Worker(std::thread&& thread) : thread(std::move(thread)), abort(false)
{
}
